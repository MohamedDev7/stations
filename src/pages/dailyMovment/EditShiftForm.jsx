...
const EditShiftForm = () => {
	...
	// const [prevStoresMovments, setPrevStoresMovments] = useState([]);
	...
	const { data: shiftData } = useQuery({
		queryKey: ["shifts", info.state.movment_id, info.state.shift_number],
		...
		select: (res) => {
			...
			let incomes = [];
			if (res.data.incomes.length > 0) {
				incomes = res.data.incomes.map((el, i) => {
					return {
						...el,
						store: el.store_id,
						db_id: el.id,
			...
			if (res.data.incomes.length > 0) {
				others = res.data.others.map((el, i) => {
					return { ...el, store: el.store_id, db_id: el.id, id: i + 1 };
				});
			}
			return { dispensers, incomes, others };
		},
		onSuccess: (data) => {
			console.log(`data`, data);
			setDispensers(data.dispensers);
			setIncomesCount(data.incomes.length);
			setIncomesIsChecked(true);
	...
	// 			return { ...el, totalLiters: 0, totalValue: 0, prev_A: el.curr_A };
	// 		});
	// 	},
	// 	onSuccess: (data) => {
	// 		if (data.length === 0) {
	...
	// 		shiftNumber: "",
	// 	});
	// 	console.log(`info.state`, info.state.shift_number);
	// 	if (info.state.shift_number === 1) {
	// 		//قراءة مناوبات اليوم السابق
	// 		console.log(`قراءة مناوبات اليوم السابق`);
	// 		refetchPrevMovment();
	// 	} else {
	...
	// 		});
	// 		console.log(`قراءة مناوبات اليوم الحالي`);
	// 	}
	// }, [info]);
	...
	const onSaveMovmentHandler = () => {
		...
		console.log(`currStoresMovments`, currStoresMovments);
	};
											...
											<TableCell>
												<div style={{ display: "flex", gap: "5px" }}>
													...
													<DefaultButton
														className={styles.deleteBtn}
														onClick={() =>
															saveStoresTransferHandler((prev) =>
																prev.filter((el) => el.id !== item.id)
															)
														}
													>
							...
							{currStoresMovments &&
								currStoresMovments.map((item, i) => (
									<TableRow key={i}>
										...
										<TableCell>{item.store.substance.name}</TableCell>
